...
public class TryMonadicTest {
    ...
    public void shouldMapMonadicTry() throws Exception {
        final Monad<Try, Integer, Try<Integer>> integerTry = monad(Try.success(1));
        final HoType<Try, Integer, Try<Integer>> mappedIntegerTry = integerTry.map(i -> i + 1);
    ...
    public void shouldFlatmapMonadicTry() throws Exception {
        final Monad<Try, Integer, Try<Integer>> integerTry = monad(Try.success(1));
        final HoType<Try, Integer, Try<Integer>> flatMappedIntegerTry = integerTry.flatmap(i -> Try.success(i + 1));
    ...
    public void shouldApplypMonadicTry() throws Exception {
        final Monad<Try, Integer, Try<Integer>> integerTry = monad(Try.success(1));
        final HoType<Try, Integer, Try<Integer>> appliedIntegerTry = integerTry.apply(monad(Try.success(i -> i + 1)));
